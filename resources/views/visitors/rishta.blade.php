    ...
    <section class="main-section masjidListSection">
        <div class="container">
            <div class="filterWrap" id="filterContainer">
                <form id="filterForm" action="{{ route('fetch_rishta_masjid') }}" method="post" onsubmit="return submitForm()">
                    @csrf
                    <button type="submit" class="btn">فلٹر</button>

                    <select name="country" id="country">
                        <option value="ملک" selected>ملک</option>
                        @foreach ($rishta->unique('country') as $key => $value)
                    ...
                    <select name="city" id="city">
                        ...
                        @foreach ($rishta->unique('city') as $key => $value)
                        ...
                        <div class="col-md-4">
                            <a href="{{ route('masjid_detail', $value->id) }}">
                                ...
                                <figure>
                                    ...
                                    <figcaption>
                                        <span> مسجد کا نام: <strong>{{ $value->full_name }}</strong>
                                        </span>
                                        <span> مسجد کا مکمل پتہ: <strong>{{ $value->gender }}</strong>
                                        </span>
            ...
            <script>
                function submitForm() {
                    ...
                    fetch(form.action, {
                        ...
                        .then(data => {
                            ...
                            updateFrontend(data.rishtaList, baseUrl);
                        })
                        .catch(error => {
                ...
                function updateFrontend(rishtaList, baseUrl) {
                    const container = document.getElementById('filteredDataContainer');
                    container.innerHTML = ''; // Clear existing content

                    rishtaList.forEach(item => {
                        const colDiv = document.createElement('div');
                        colDiv.className = 'col-md-4';

                        const aTag = document.createElement('a');
                        aTag.href = `${baseUrl}/rishta/detail/${item.id}`;
                        ...
                        nameSpan.innerHTML = `مسجد کا نام: <strong>${item.full_name}</strong>`;
                        figcaption.appendChild(nameSpan);
                        const addressSpan = document.createElement('span');
                        addressSpan.innerHTML = `مسجد کا مکمل پتہ: <strong>${item.gender}</strong>`;
